...
export async function GET(request: NextRequest) {
  ...
  if (!(await rateLimiter.canMakeRequest())) {
    ...
    return NextResponse.json(
      fallbackProblems[arena as keyof typeof fallbackProblems] ||
        fallbackProblems.arena1
    );
  }
  ...
  try {
    ...
    const difficulty =
      difficultyMap[arena as keyof typeof difficultyMap] || "Easy";
...
- IMPORTANT: - The field "boilerplate_code" must contain a **complete Java program**.
- The full Java program must:
  - Use a public class named Main
  - Contain a public static void main(String[] args) method
  - Read inputs using Scanner from System.in
  - Manually parse inputs from string to data structures (like List<List<Integer>>)
  - Call the user-defined method
  - Print the final result to System.out
  - DO NOT PROVIDE THE SOLUTION TO THE PROBLEM (THAT IS TO BE INPUT BY THE USER)

Example format of boilerplate_code:

java
import java.util.*;

public class Main {
    public static int treeDiameter(List<List<Integer>> adj) {
        // your logic
    }

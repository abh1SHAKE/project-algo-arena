...
const Timer: React.FC = () => {
  ...
  return (
    <div
      className={`${styles["timer-container"]} flex-row align-items-center justify-content-center`}
    >
      {showTimer ? (
        <div className={`${styles["timer-active-state"]} flex-row gap-8`}>
          <div
            className={`${styles["pause-icon"]} flex-row cursor-pointer`}
            onClick={togglePause}
          >
            {isRunning ? (
              <svg
          ...
          <div
            className={`${styles["reset-timer-icon"]} flex-row cursor-pointer`}
            onClick={resetTimer}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
        ...
        <Tooltip text="Timer" position="left">
          <div
            className={`${styles["timer-inactive-state"]} ${
              showTimer ? styles["expanded"] : ""
            } flex-row gap-4 cursor-pointer`}
            onClick={() => {
              setShowTimer(true);
              startTimer();
            }}
